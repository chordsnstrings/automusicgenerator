...
- **Sampling is not portable.** Current Claude models REMOVED `temperature`,
  `top_p` and `top_k` — sending one returns a 400. Every agent here passes a
  temperature, so the Anthropic backend drops it rather than forwarding it; the
