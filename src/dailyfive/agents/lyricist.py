...
Runs on MiniMax M3. ModelArk, which was the original suggestion, exposes no
text models at all — it is image, video and 3D only — so the lyric work lives
here and ModelArk earns its place doing cover art instead.
...
def run(brief: dict, *, client: MiniMaxClient | None = None) -> dict:
    ...
    for n in (1, 2):
        try:
            text = client.chat(SYSTEM, prompt + _variation(n, slot),
    ...
    if not drafts:
        raise ProviderError("minimax", f"no usable lyric draft for {brief.get('title')!r}",
                            retryable=True)

    if len(drafts) == 1:
        chosen, which = drafts[0], 1
    else:
        chosen, which = _choose(client, brief, drafts)
...
def _choose(client: MiniMaxClient, brief: dict, drafts: list[str]) -> tuple[str, int]:
    ...
    try:
        answer = client.chat(system, user, max_tokens=8, temperature=0.0)
    except ProviderError as exc:
